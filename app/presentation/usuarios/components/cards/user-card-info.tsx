...
export function UserCardInfo({ user }: UserCardInfoProps) {
  ...
  return (
    <Card className="w-full max-w-sm shadow-xl relative z-10 overflow-hidden py-0">
      ...
      <CardFooter className="p-2 bg-primary/10 dark:bg-primary/10 rounded-b-xl cursor-pointer hover:bg-primary/20 dark:hover:bg-primary/20 transition-all duration-200">
        <Link
          to={`/usuarios/ver/${user.id}`}
          className="flex items-center justify-center space-x-2 text-xs sm:text-sm font-medium text-primary py-2 sm:py-3 w-full"
        >
